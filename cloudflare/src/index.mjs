...
export function renderMarkdown(handoff, sections) {
  ...
  if (handoff.context?.available) {
    lines.push(
      "### Attached Context",
      "",
      `完整的可读会话已在尽力脱敏后附带。需要核对细节时，请运行 \`handoff context handoff:${handoff.id}\` 按需读取；它不是原始 Provider Session。`,
      "",
    );
  }
